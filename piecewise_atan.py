...
x_range = (-1, 1)    # Range for the atan function
num_segments = 3            # Number of segments to approximate the atan function
segment_breakpoints = [-1, -0.5, 0.5, 1]

# Generate atan function values for the given range
x_values = np.linspace(x_range[0], x_range[1], 1000)
y_values = np.arctan(x_values)

# Squish the atan wave into a range from -1 to 1
x_range = (-1, 1)
x_values = np.linspace(x_range[0], x_range[1], 1000)
...
plt.plot(x_values, y_values, label='Atan Function', color='blue')
plt.plot(x_values, y_approx, label='Piecewise Linear Approximation', color='red', linestyle='dashed')
plt.xlabel('x')
plt.ylabel('y')
plt.title('Piecewise Linear Approximation of Atan Function')
plt.legend()
plt.grid(True)
plt.show()