...
x_range = (-1 * np.pi, 1 * np.pi)    # Range for the cosine function
num_segments = 7                     # Number of segments to approximate the cosine function
# segment_breakpoints = [-1, -0.5, 0.5, 1]
segment_breakpoints = [-1 * np.pi, -0.8 * np.pi, -0.25 * np.pi,  -0.05 * np.pi, 0.05 * np.pi, 0.25 * np.pi, 0.8 * np.pi, 1 * np.pi]

# Generate cosine function values for the given range
x_values = np.linspace(x_range[0], x_range[1], 1000)
y_values = np.cos(x_values)

# Squish the cosine wave into a range from -1 to 1
# x_range = (-1, 1)
# x_values = np.linspace(x_range[0], x_range[1], 1000)
...
plt.plot(x_values, y_values, label='Cosine Function', color='blue')
plt.plot(x_values, y_approx, label='Piecewise Linear Approximation', color='red', linestyle='dashed')
plt.xlabel('x')
plt.ylabel('y')
plt.title('Piecewise Linear Approximation of Cosine Function')
plt.legend()
plt.grid(True)
plt.show()